...
The purpose of the Simple Evolutionary Exploration for segmentation, or SEE-Segment, software package is to provide an easy-to-use tool that can search for image segmentation algorithms. The software vectorizes the algorithms and their hyperparaameters and uses Genetic Algorithms to search this space.  A Labeled Array Distance metric [@colbry2020] has been developed to help guild the search.  The resulting solution can be viewed as documented example python code with only minimal/standard dependencies (Numpy, Pillow, scikit-image, etc.) in an effort to make the resulting solution as portable and useful as possible. 
...
More specifically SEE-Segment takes full advantage of the Python DEAP [@Fortin:2012] library for conducting the Genetic Search.  The library keeps track of the current "population" of best algorithms and helps with generating new individuals though random selection and various forms of mutation.  
